...
class ResNetFPN(nn.Module):
    def __init__(self, backbone, heads, head_conv=128,
        ...
        if backbone == 'resnet18':
            ...
            pretrained = 'imagenet' if pretrained else None
            self.backbone = pretrainedmodels.resnext101_32x4d(pretrained=pretrained)
            num_bottleneck_filters = 2048
...
# %% [code]
def validate(heads, val_loader, model, criterion):
    avg_meters = {'loss': AverageMeter()}
    for head in heads.keys():
        avg_meters[head] = AverageMeter()
...
name = f'{backbone}_fpn'
arch = f'{backbone}_fpn'
if nrows:
...
for fold, (train_idx, val_idx) in enumerate(kf.split(img_paths)):
    ...
    # 创建记录文件
    txtName = f"./log/log_{backbone}_fold{fold}.txt"
    f = open(txtName, "w")
    log_loss = []
    log_val_loss = []
    for epoch in range(EPOCHES):
        ...
        if already_load_model == False:
            if os.path.exists('./model/model_{}_{}.pth'.format(name,fold+1)):
                ...
                best_loss = validate(heads,val_loader,model,criterion)
        ...
        val_loss = validate(heads, val_loader, model, criterion)
